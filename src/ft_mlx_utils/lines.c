...
static void	draw_rect_with_alpha(t_ftm_image *canvas, SDL_Rect rect, unsigned int color)
{
    ...
    for (y = rect.y; y < rect.y + rect.h && y < canvas->size.height; y++)
    {
        if (y < 0) continue;
        for (x = rect.x; x < rect.x + rect.w && x < canvas->size.width; x++)
        {
            if (x < 0) continue;
